...
def runActiveLearningScheme(
    rootFolder: str, config: dict, initialTrainingSet: str, mtpLevel="08"
):
    ...
    # Only copy the training set if it's a new run
    if len(existingArchiveFolders) == 0:
        shutil.copy(initialTrainingSet, trainingFile)
        wr.printAndLog(logFile, "Starting Fresh Run!")
    ...
    # Otherwise, we are continuing a run, and we should parse where we were
        ...
        for archive in existingArchiveFolders:
            ...
            if score > maxScore:
                attempt, startingIter = tuple(map(int, archive.split("_")))  # Extract
                maxScore = score
    ...
    wr.printAndLog(
        logFile,
        "Starting Active Learning Process !!! Attempt: "
        + str(attempt)
        + "\t Iteration: "
        + str(startingIter),
    )
    ...
    for iteration in range(startingIter, config["maxIters"], 1):

        wr.printAndLog(
            logFile,
            "Entering Iteration " + str(iteration) + ".",
        )

        ### ===== Train the MTP ====
        wr.printAndLog(logFile, "Starting Training Stage.")
        rmsEnergyError, rmsForceError, trainingTime = trainMTP(
        ...
        wr.printAndLog(logFile, "Average Energy Per Atom Error: " + rmsEnergyError)
        wr.printAndLog(logFile, "Average Force Per Atom Error: " + rmsForceError)
